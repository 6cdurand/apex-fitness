...
export async function fetchAllUserDataFromSupabase(userId: string): Promise<{
  ...
  try {
    const [coreData, messageData, following, followers, strengthRating] = await Promise.all([
...
// ============================================================================
// FOLLOW SYSTEM SYNC (v9-06 - canonical user_follows table)
// ============================================================================
...
export async function syncFollowToSupabase(
  ...
  try {
    ...
    if (error) {
      ...
      // Schema-drift retry: table doesn't exist yet
      if (errCode === '42P01' || errMsg.includes('relation') && errMsg.includes('does not exist')) {
        console.warn(
          '[FollowSync] ⚠️ user_follows table missing — apply 20260512 migration.',
          'Follows will save to localStorage only until migration applied.'
        );
        return { success: false, warning: 'user_follows table missing — apply 20260512 migration' };
      }

      // Column doesn't exist
      if (errCode === '42703' || errMsg.includes('column') && errMsg.includes('does not exist')) {
        console.warn(
          '[FollowSync] ⚠️ Schema drift detected on user_follows.',
          { code: errCode, message: error.message }
        );
        return { success: false, warning: 'Schema drift on user_follows' };
      }

      // Other errors (e.g., duplicate, constraint violation)
      console.error('[FollowSync] Error syncing follow:', error);
      return { success: false };
    }

    return { success: true };
  } catch (err) {
    console.error('[FollowSync] Exception syncing follow:', err);
    return { success: false };
  }
}

export async function syncUnfollowFromSupabase(
  followerId: string,
  followeeId: string
): Promise<{ success: boolean; warning?: string }> {
  try {
    ...
    if (error) {
      ...
      // Schema-drift: table doesn't exist yet
      if (errCode === '42P01' || errMsg.includes('relation') && errMsg.includes('does not exist')) {
        console.warn('[FollowSync] ⚠️ user_follows table missing — apply 20260512 migration.');
        return { success: false, warning: 'user_follows table missing' };
      }

      console.error('[FollowSync] Error syncing unfollow:', error);
      return { success: false };
    }

    return { success: true };
  } catch (err) {
    console.error('[FollowSync] Exception syncing unfollow:', err);
    return { success: false };
  }
}

export async function fetchFollowingFromSupabase(
  followerId: string
): Promise<string[]> {
  try {
    const { data, error } = await getWorkoutClient()
    ...
    if (error) {
      ...
      if (errCode === '42P01' || errMsg.includes('relation') && errMsg.includes('does not exist')) {
        console.warn('[FollowSync] ⚠️ user_follows table missing — returning empty following list.');
        return [];
      }

      console.error('[FollowSync] Error fetching following:', error);
      return [];
    }

    // Dedupe and return just IDs
    return Array.from(new Set((data || []).map((row: any) => row.followee_id)));
  } catch (err) {
    console.error('[FollowSync] Exception fetching following:', err);
    return [];
  }
}
...
export async function fetchFollowersFromSupabase(
  ...
  try {
    ...
    if (error) {
      ...
      if (errCode === '42P01' || errMsg.includes('relation') && errMsg.includes('does not exist')) {
        console.warn('[FollowSync] ⚠️ user_follows table missing — returning empty followers list.');
        return [];
      }

      console.error('[FollowSync] Error fetching followers:', error);
      return [];
    }

    // Dedupe and return just IDs
    return Array.from(new Set((data || []).map((row: any) => row.follower_id)));
  } catch (err) {
